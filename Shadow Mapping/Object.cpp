...
int Object::readMaterial()
{
	...
	currentma.texImg = new TextureImage();
	LoadTGA(currentma.texImg,currentma.texture.c_str());
	masMap[currentma.name]=currentma;
	return 0;
}
	...
	glPushMatrix();
		...
		glTranslatef(x/s,y/s,z/s);
			for(int j=0;j<num_s;j++)
			{
				...
				glBindTexture(GL_TEXTURE_2D,setmaterial.texImg->texID);
