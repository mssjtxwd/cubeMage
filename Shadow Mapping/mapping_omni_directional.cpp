...
GLuint tex_walls;
GLuint tex_shadow[6]; // 立方体的6个面，全方向光源
...
void tex_init()
{
	...
	for(int i=0; i<6; ++i){
		...
		// 分配纹理像素的存储空间，见 OpenGL 3.3 (Compatibility Profile) Specification pp.237
...
void draw(const glm::mat4& mat_model, const glm::mat4& mat_view)
{
	...
	GLboolean li = glIsEnabled(GL_LIGHTING); // 不需要光照和纹理
	if(li) glDisable(GL_LIGHTING);
